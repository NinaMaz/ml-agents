...
class PPOModel(LearningModel):
    ...
    def create_ppo_optimizer(self, probs, old_probs, value, entropy, beta, epsilon, lr, max_step):
        ...
        vpredclipped = self.old_value + tf.clip_by_value(tf.reduce_sum(value, axis=1) - self.old_value,
                                                         - decay_epsilon, decay_epsilon)

        v_opt_a = tf.squared_difference(self.returns_holder, tf.reduce_sum(value, axis=1))
        v_opt_b = tf.squared_difference(self.returns_holder, vpredclipped)
        ...
        self.loss = self.policy_loss + 0.5 * self.value_loss - decay_beta * tf.reduce_mean(tf.boolean_mask(entropy, self.mask))
